package com.alilopez.kt_demohilt.features.issues.domain.usecases

import com.alilopez.kt_demohilt.features.issues.domain.entities.Issue
import com.alilopez.kt_demohilt.features.issues.domain.repositories.IssuesRepository
import javax.inject.Inject
import kotlin.time.Duration

class UpdateCurrentTimeUseCase @Inject constructor(private val repository: IssuesRepository) {
    suspend operator fun invoke(id: Long, currentTime: Duration): Result<Issue> {
        return try {
            val issue = repository.updateCurrentTime(id, currentTime)
            Result.success(issue)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
}